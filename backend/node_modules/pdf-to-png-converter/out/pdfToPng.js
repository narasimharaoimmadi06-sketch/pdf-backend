...
async function pdfToPng(pdfFile, props) {
    ...
    try {
        const pageViewportScale = props?.viewportScale !== undefined
            ? props.viewportScale
        ...
        if (props?.processPagesInParallel === true) {
            pngPageOutputs.push(...(await Promise.all(validPagesToProcess.map((pageNumber) => processPdfPage(pdfDocument, props?.outputFileMaskFunc?.(pageNumber) ?? `${defaultMask}_page_${pageNumber}.png`, pageNumber, pageViewportScale, props?.returnPageContent ?? true)))));
        }
        else {
            for (const pageNumber of validPagesToProcess) {
                pngPageOutputs.push(await processPdfPage(pdfDocument, props?.outputFileMaskFunc?.(pageNumber) ?? `${defaultMask}_page_${pageNumber}.png`, pageNumber, pageViewportScale, props?.returnPageContent ?? true));
            }
        }
 ...
 */
async function processPdfPage(pdf, pageName, pageNumber, pageViewportScale, returnPageContent) {
